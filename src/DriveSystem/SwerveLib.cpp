...
SwerveLib::SwerveLib(double width, double length) {
	...
	// Note: pow is inefficient for squaring
	_radius = sqrt((width * width) + (length * length));
	_width = width;
	_length = length;
...
void SwerveLib::CalcWheelVect(double mag,
	...
	// Calculate the wheel speeds. Only Pythagorean Theorem
	// Note: pow is inefficient for squaring
	_targetWhlSpeed_RF = sqrt((_B * _B) + (_C * _C));
	_targetWhlSpeed_LF = sqrt((_B * _B) + (_D * _D));
	_targetWhlSpeed_LB = sqrt((_A * _A) + (_D * _D));
	_targetWhlSpeed_RB = sqrt((_A * _A) + (_C * _C));
